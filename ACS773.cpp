#include "ACS773.h"
int zero; 
ACS773::ACS773(ACS773_type type, uint8_t _pin) {
	switch (type) {
		case ACS773LCB_050B:
			sensitivity = 0.0264;
			break;
	    case ACS773LCB_100B:
			sensitivity = 0.0132;
			break;
		case ACS773KCB_150B:
			sensitivity = 0.0088;
			break;
		case ACS773ECB_200B:
			sensitivity = 0.0066;
			break;
		case ACS773ECB_250U:
			sensitivity = 0.01056;
			break;
		default:
			sensitivity = 0.0088;
			break;
	}
	pin = _pin;
}

int ACS773::calibrate() {
	int _zero = 0;
	for (int i = 0; i < 10; i++) {
		_zero += analogRead(pin);
		delay(10);
	}
	_zero /= 10;
	zero = _zero;
	return _zero;
}

void ACS773::setZeroPoint(int _zero) {
	zero = _zero;
}

void ACS773::setSensitivity(float sens) {
	sensitivity = sens;
}

float ACS773::getCurrentDC() {
	float I = (zero - analogRead(pin)) / ADC_SCALE * VREF / sensitivity;
	return I;
}

float ACS773::getCurrentAC() {
	return getCurrentAC(DEFAULT_FREQUENCY);
}

float ACS773::getCurrentAC(uint16_t frequency) {
	uint32_t period = 1000000 / frequency;
	uint32_t t_start = micros();

	uint32_t Isum = 0, measurements_count = 0;
	int32_t Inow;

	while (micros() - t_start < period) {
		Inow = zero - analogRead(pin);
		Isum += Inow*Inow;
		measurements_count++;
	}

	float Irms = sqrt(Isum / measurements_count) / ADC_SCALE * VREF / sensitivity;
	return Irms;
}
